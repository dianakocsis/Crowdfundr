...
const abi = require("../artifacts/contracts/Crowdfunding.sol/Project.json").abi;
...
describe("Crowdfunding Contract", function () {
    ...
    beforeEach(async function () {
        ...
        [projectAddress] = await crowdfunding.connect(owner).getRegisteredProjects(); // get access to project address that was created
    ...
    describe("Contributing", function () {
        ...
        it('Requires a minimum amount of ether to contribute', async function () {
            await expect(project.connect(addr1).contribute({
        ...
        it('Cannot contribute if goal has been reached', async function () {
            ...
            await expect(project.connect(addr2).contribute({
        ...
        it("Cannot contribute after 30 days", async function () {
            ...
            await expect(project.connect(addr1).contribute()).to.be.revertedWith("Cannot be called at this time.");
        })
    ...
    describe("Withdrawing", function () {
        ...
        it("Only creator can withdraw", async function () {
            await expect(project.connect(addr1).withdraw(50)).to.be.revertedWith("Not creator");
        });
        ...
        it("Proper amount is withdrawed", async function () {
            ...
            expect(await ethers.provider.getBalance(projectAddress)).to.be.equal(ethers.utils.parseEther("7.5"));
        })
    ...
    describe("Refunding", function () {
        ...
        it("Can't receive refund if did not contribute", async function () {
            ...
            await project.connect(owner).cancel();
            await expect(project.connect(addr1).refund()).to.be.revertedWith("did not contribute");
        });
        ...
        it("Cannot receive refund if project is still funding", async function () {
            ...
            await expect(project.connect(addr1).refund()).to.be.revertedWith("Cannot be called at this time.");
        })
        ...
        it("Can receive refund if deadline passed and goal not reached", async function () {
            ...
            expect(await ethers.provider.getBalance(projectAddress)).to.be.equal(ethers.utils.parseEther("5"));
            expect(await project.connect(owner).getContribution(addr1.address)).to.be.equal(ethers.utils.parseEther("5"));
            ...
            expect(await ethers.provider.getBalance(projectAddress)).to.be.equal(ethers.utils.parseEther("0"));
            expect(await project.connect(owner).getContribution(addr1.address)).to.be.equal(ethers.utils.parseEther("0"));
    ...
    describe("Cancelling", function () {
        ...
        it("Can only be cancelled by creator", async function () {
            await expect(project.connect(addr1).cancel()).to.be.revertedWith("Not creator");
        })
        ...
        it("Can't be cancelled if fully funded", async function () {
            ...
            await expect(project.connect(owner).cancel()).to.be.revertedWith("Cannot be called at this time.");
        })
    ...
    describe("NFT", function () {
        ...
        it('Tiers', async function () {
            ...
            expect(await project.connect(owner).ownerOf(6)).to.be.equal(addr2.address);
        ...
        it("Balance of tokens", async function () {
            ...
            expect(await project.connect(owner).balanceOf(addr1.address)).to.be.equal(3);
        })
